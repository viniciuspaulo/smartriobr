...
abstract class ingalleryControllerBackend
{
	...
	static function bind_admin_print_styles(){
		...
		wp_localize_script('ingallery-plugin', 'ingallery_ajax_object',array( 
			...
			'lang' => array(
                ...
                'ordering' => __('Ordering','ingallery'),
                'equalalbumitems' => __('Albums load items equal amount','ingallery'),
                'ordering_help' => __('When "Albums load items equal amount" is selected, then the plugin will try to load equal amount of items for each album','ingallery'),
				'rtl' => __('RTL (right-to-left)','ingallery'),
                'local_responsive' => __('Local responsiveness','ingallery'),
			)
		));
	}
	...
	static function handleFrontendComments(){
		try{
			...
			if(!isset($items[0]['edge_media_to_comment']) || !isset($items[0]['edge_media_to_comment']['edges']) || !is_array($items[0]['edge_media_to_comment']['edges']) || count($items[0]['edge_media_to_comment']['edges'])==0){
				throw new Exception(__('No comments found','ingallery'));
			}
            
            $comments = array();
            foreach($items[0]['edge_media_to_comment']['edges'] as $edge){
