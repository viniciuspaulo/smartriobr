...
class ingalleryModel
{
	...
	private $_data = array(
			...
			'cfg' => array(
				...
				'layout_rtl' => 0,
                'layout_local_responsive' => 0,
				'display_style' => 'default',
                'display_ordering' => 'created_date',
	...
	public function getGalleryCacheLifetime(){
		$minAlbumCache = 36000;
		foreach($this->get('albums') as $album){
			if((int)$album['cache_lifetime']<$minAlbumCache){
				$minAlbumCache = (int)$album['cache_lifetime']+1;
			}
		}
		return $minAlbumCache;
	}
	...
	public function loadItems(){
        ...
        if($this->get('cfg/display_ordering')=='equalalbumitems'){
            $this->distibItemsAlbumsEqual();
        }
		
		return true;
	}
    
    public function distibItemsAlbumsEqual(){
        $items = $this->_items;
        $newStack = array();
        $itemsPerPage = (int)$this->get('cfg/layout_cols')*(int)$this->get('cfg/layout_rows');
        $itemsPerAlbumPerPage = floor($itemsPerPage/count($this->get('albums')));
        $albums = $this->get('albums');

        while(count($items)){
            foreach($albums as $album){
                for($i=0;$i<$itemsPerAlbumPerPage;$i++){
                    $x = count($items);
                    $nextItem = self::getNextAlbumItem($items,$album);
                    if(!is_null($nextItem)){
                        $newStack[] = $nextItem;
                    }
                }
    ...
    public static function getNextAlbumItem(&$items,$album){
		...
		if(isset($input['albums']) && is_array($input['albums'])){
			foreach($input['albums'] as $inputAlbum){
				...
				if(isset($inputAlbum['sources'])){
					$album['sources'] = ingalleryHelper::sourcesToArray($inputAlbum['sources']);
				}
				if(isset($inputAlbum['filters'])){
					if(isset($inputAlbum['filters']['only'])){
						$album['filters']['only'] = ingalleryHelper::sourcesToArray($inputAlbum['filters']['only'],array('videos'));
					}
					if(isset($inputAlbum['filters']['except'])){
						$album['filters']['except'] = ingalleryHelper::sourcesToArray($inputAlbum['filters']['except'],array('videos'));
					}
				}
	...
	public function updateChecksum(){
		$this->_checksum = md5(serialize($this->_data));
	}
